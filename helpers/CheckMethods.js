...
export const checkExist = async (id, Model, extraQuery = {}, errorMessage = '') => {
    ...
    throw new ApiSuccess(false,400, errorMessage || `${Model.modelName} Not Found`,{});

};
export const checkCouponExist = async (couponNumber, Model, extraQuery = {}, errorMessage = '') => {
    if (typeof extraQuery != 'object') {
        errorMessage = extraQuery;
        extraQuery = {};
    }

    if (validId(couponNumber)) {
        let model = await Model.findOne({ couponNumber: couponNumber, ...extraQuery }).lean();
        if (model)
            return;
    }
